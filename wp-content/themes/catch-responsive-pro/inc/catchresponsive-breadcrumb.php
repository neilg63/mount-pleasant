...
if ( !function_exists( 'catchresponsive_custom_breadcrumbs' ) ) :
	...
	function catchresponsive_custom_breadcrumbs( $showOnHome, $delimiter ) {
		...
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'catch-responsive-pro' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'catch-responsive-pro' ); // text for a category page
		$text['search']   = __( '%1$s Search results for: %2$s', 'catch-responsive-pro' ); // text for a search results page
		$text['tag']      = __( '%1$s Posts tagged %2$s', 'catch-responsive-pro' ); // text for a tag page
		$text['author']   = __( '%1$s View all posts by %2$s', 'catch-responsive-pro' ); // text for an author page
		$text['404']      = __( 'Error 404', 'catch-responsive-pro' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( __( 'Page %s', 'catch-responsive-pro' ), max( $paged, $page ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
